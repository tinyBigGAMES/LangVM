...
resourcestring
  ...
  RSLVMBuiltinArgType    = '%s() %s must be a %s';
  RSLVMBuiltinFailed     = '%s(): %s';
  RSLVMBuiltinInvHandle  = '%s(): invalid handle %d';
  RSLVMBufOutOfBounds    = '%s(): buffer range [%d..%d) out of bounds (size %d)';
  RSLVMIndexOutOfRange   = '%s(): index %d out of range [0..%d)';
  RSLVMMirNoModule       = '%s(): no module in progress';
  RSLVMMirNoFunc         = '%s(): no function in progress';
  RSLVMMirNested         = '%s(): nested %s not allowed';
  RSLVMMirUnknownType    = '%s(): unknown %s type "%s"';
  RSLVMDllLoadFailed     = 'loadDll(): LoadLibrary failed for "%s" (error %d)';
  RSLVMDllProcFailed     = 'getDllProc(): GetProcAddress failed for "%s" (error %d)';
  RSLVMImportNotFound    = 'Import file not found: %s';
  RSLVMFragmentNotFound  = 'Fragment not found: %s';
  RSLVMLangNotFound      = 'Language file not found: %s';
  RSLVMUndefRoutine      = 'Undefined routine: "%s"';
  RSLVMNotRoutine        = '"%s" is not a routine';
  RSLVMArgTypeMismatch   = 'Argument %d: expected %s, got %s';
  RSLVMGrammarNotFound   = 'RunGrammarRule: no grammar rule "%s"';
  RSLVMVarRedeclared     = 'Variable ''%s'' already declared in this scope';
  RSLVMVarUndeclared     = 'Undeclared variable ''%s''';
  RSLVMTypeMismatch      = 'Cannot assign to variable ''%s'' -- type mismatch';
  RSLVMCannotAssignField = 'Cannot assign field "%s" on value of kind %s';
  RSLVMNoSuchField       = 'Record "%s" has no field "%s"';
  RSLVMListIndexBounds   = 'List index out of bounds: %d';
  RSLVMCannotAssignIndex = 'Cannot assign by index on value of kind %s';
  RSLVMInvalidAssignTarget = 'Invalid assignment target "%s"';
  RSLVMWhileLimit        = 'While loop exceeded iteration limit';
  RSLVMCannotIterate     = 'Cannot iterate over %s';
  RSLVMReturnWithValue   = 'Return with value in routine "%s" which has no declared return type (add -> type)';
  RSLVMBareReturn        = 'Bare return in routine "%s" which declares return type "%s"';
  RSLVMUndefVar          = 'Undefined variable: %s';
  RSLVMUnknownUnaryOp    = 'Unknown unary operator: %s';
  RSLVMUnknownBinaryOp   = 'Unknown binary operator: %s';
  RSLVMNotCallable       = 'Not callable: %s';
  RSLVMUnknownFunc       = 'Unknown function: %s';
  RSLVMCannotCallExpr    = 'Cannot call expression result';
  RSLVMStrIndexBounds    = 'String index out of bounds: %d';
  RSLVMCannotIndex       = 'Cannot index value of kind %s';
  RSLVMCannotAccessField = 'Cannot access field "%s" on value of kind %s';
  RSLVMUnknownLayoutType = 'Unknown layout field type: %s';
